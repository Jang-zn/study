...
public class SearchMemberListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String column = request.getParameter("searchType");
		String value = request.getParameter("searchKeyword");
		// 페이징 처리를 위한 전처리 - common class 만들어놓고 쓰면 편함
		...
		// 페이지바 변수 설정
		int totalData = new AdminService().getMemberCount(column, value);
		...
		if (pageNo == 1) {
			...
			pageBar += "<a href='" + request.getContextPath() + "/admin/searchMemberList?cPage=" + (pageNo - 1)
					+ "&numPerpage=" + numPerpage + "&searchType=" + column + "&searchKeyword=" + value + "'>[이전]</a>";
		}
		...
		while (!(pageNo > pageEnd || pageNo > totalPage)) {
			if (cPage == pageNo) {
				...
				pageBar += "<a href='" + request.getContextPath() + "/admin/searchMemberList?cPage=" + pageNo
						+ "&numPerpage=" + numPerpage + "&searchType=" + column + "&searchKeyword=" + value
						+ "'>&nbsp&nbsp" + pageNo + "&nbsp&nbsp</a>";
			}
			pageNo++;
		}
		...
		if (pageNo > totalPage) {
			...
			pageBar += "<a href='" + request.getContextPath() + "/admin/searchMemberList?cPage=" + pageNo
					+ "&numPerpage=" + numPerpage + "&searchType=" + column + "&searchKeyword=" + value + "'>[다음]</a>";
		}

		request.setAttribute("pageBar", pageBar);

		List<Member> list = new AdminService().searchMemberList(column, value, cPage, numPerpage);
		for (Member m : list) {
			try {
